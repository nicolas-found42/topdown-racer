...
- [ ] Exactly one challenge ships, on the sequence delivered by #53; there are no AI Opponents in the practice session.
...
This is a focused post-v1 practice-mode addition. Do not add ghosts, replay recording, medals, multiple selectable sections, a Track selector, or automatic driving advice. Reuse authoritative directional gates and record identity from #50, but keep challenge attempts distinct from the domain's three-lap Race. Add only the domain vocabulary needed to explain that distinction.

## Blocked by

- #50 — Match lap timing to the checker and preserve eligible Race results (reliable gates and record eligibility/identity).
- #53 — Give the hillside Track a readable braking duel and direction-change sequence (the single authored challenge target).
