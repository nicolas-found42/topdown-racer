use topdown_racer_core::{
    simulation::{CarInput, DrivingMode, Sim},
...
fn explicit_ownership_applies_at_tick_boundary_and_autopilot_ignores_keys() {
    ...
    for _ in 0..120 {
        assert_eq!(sim.tick(&[])[0].pose, initial);
    }
...
fn assistance_sticks_to_the_lap_but_next_manual_flying_lap_can_set_a_record() {
    ...
    for _ in 0..6000 {
        ...
        let input = scripted_driver.compute_input(AiView {
            active: None,
            car_index: 0,
            pose: snap.pose,
...
fn identical_preset_and_mode_input_streams_replay_identical_snapshots() {
    ...
    for pace in [
        ...
        for tick in 0..4500 {
            if let Some(mode) = match tick {
                0 | 1800 => Some(DrivingMode::Autopilot),
                1600 | 3300 => Some(DrivingMode::Manual),
                _ => None,
            } {
            ...
            let input = CarInput {
                throttle: if tick % 200 < 100 { 1.0 } else { 0.0 },
                steer: 0.1,
                ..Default::default()
            };
            assert_eq!(a.tick(&[input]), b.tick(&[input]), "{pace:?} tick {tick}");
            for car in 1..4 {
                assert!(a.ai_enabled(car));
            }
        }
    }
}
