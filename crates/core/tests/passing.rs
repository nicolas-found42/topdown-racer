use glam::Vec2;
use topdown_racer_core::{ai::{AiDriver, AiView}, track::{Track, SAMPLE_CIRCUIT}};
...
fn committed_pass_does_not_swap_sides_when_the_rival_moves_across_center() {
    ...
    for lateral in [0.6, -0.6, 0.6, -0.6] {
        for _ in 0..24 {
            let input = driver.compute_input(AiView { car_index: 0, pose: me.0, heading: 0.0,
                velocity: me.1, track: &track,
                field: &[me, (Vec2::new(56.0, lateral), Vec2::new(12.0, 0.0))] });
            assert!(input.steer < 0.0, "committed right pass must not switch left: {input:?}");
        }
    }
}
...
fn blocked_committed_route_aborts_and_follows_without_switching_sides() {
    ...
    for _ in 0..20 {
        driver.compute_input(AiView { car_index: 0, pose: me.0, heading: 0.0, velocity: me.1,
            track: &track, field: &[me, lead] });
    }
    let blocker = (Vec2::new(48.0, -4.0), Vec2::new(5.0, 0.0));
    for _ in 0..60 {
        let input = driver.compute_input(AiView { car_index: 0, pose: me.0, heading: 0.0, velocity: me.1,
            track: &track, field: &[me, lead, blocker] });
        assert!(input.brake > 0.5, "blocked pass must brake: {input:?}");
        assert!(input.steer <= 0.0, "abort must not switch directly to the other side");
    }
...
fn faster_car_completes_clean_pass_across_the_track_seam() {
    use topdown_racer_core::{simulation::Sim, ai::OpponentPace};
    ...
    for tick in 0..1800 {
        let snaps = sim.tick(&[]);
        if snaps[0].pose.distance(snaps[1].pose) < 28.0 { near_ticks += 1; }
        contacts += snaps.iter().filter(|s| s.car_contact).count();
        severity = severity.max(snaps.iter().map(|s| s.car_contact_speed).fold(0.0, f32::max));
    ...
    panic!("no pass completed: {:?}; contacts={contacts}, severity={severity}", sim.pass_stats(1));
}
...
fn established_inside_overlap_prevents_turning_across_the_other_car() {
    ...
    for ahead in [-4.4_f32, 0.0, 4.4] {
        ...
        for _ in 0..40 {
            let input = driver.compute_input(AiView { car_index: 0, pose: me.0, heading: 0.0,
                velocity: me.1, track: &track, field: &[me, rival] });
            assert!(input.steer <= 0.02, "overlap at {ahead} must keep the outside lane: {input:?}");
        }
    }
}
...
fn no_safe_pass_corridor_waits_without_reverse_or_repeated_attempts() {
    ...
    for _ in 0..300 {
        let input = driver.compute_input(AiView { car_index: 0, pose: me.0, heading: 0.0,
            velocity: me.1, track: &track, field: &[me, lead] });
    ...
    let clear = driver.compute_input(AiView { car_index: 0, pose: me.0, heading: 0.0,
        velocity: me.1, track: &track, field: &[me] });
    assert!(clear.throttle > 0.0, "drive as soon as the obstruction leaves");
}
...
fn late_braking_approach_slows_before_reaching_the_minimum_gap() {
    ...
    assert!(input.brake > 0.5, "brake while there is still stopping distance: {input:?}");
...
fn inside_blockage_selects_the_clear_outside_route() {
    ...
    let mut driver = AiDriver::new(1);
    let me = (Vec2::new(40.0, 0.0), Vec2::new(20.0, 0.0));
    let field = [me, (Vec2::new(56.0, 0.0), Vec2::new(10.0, 0.0)),
        (Vec2::new(55.0, 4.0), Vec2::new(10.0, 0.0))];
    for _ in 0..30 {
        let input = driver.compute_input(AiView { car_index: 0, pose: me.0, heading: 0.0,
            velocity: me.1, track: &track, field: &field });
...
fn slower_rejoin_into_a_committed_route_aborts_before_overlap() {
    ...
    let me = (Vec2::new(40.0, 0.0), Vec2::new(20.0, 0.0));
    let lead = (Vec2::new(56.0, 0.0), Vec2::new(10.0, 0.0));
    driver.compute_input(AiView { car_index: 0, pose: me.0, heading: 0.0,
        velocity: me.1, track: &track, field: &[me, lead] });
    let rejoin = (Vec2::new(52.0, -8.0), Vec2::new(5.0, 4.0));
    let input = driver.compute_input(AiView { car_index: 0, pose: me.0, heading: 0.0,
        velocity: me.1, track: &track, field: &[me, lead, rejoin] });
