use glam::Vec2;
use topdown_racer_core::{ai::AiDriver, simulation::{CarInput, GridCar, Sim}, track::Track};
...
fn car(x: f32, y: f32, speed: f32) -> GridCar {
    GridCar { pose: Vec2::new(x,y), heading: 0.0, velocity: Vec2::new(speed,0.0) }
}

#[test]
fn inside_blockage_leaves_a_clean_outside_pass_in_motion() {
    let mut sim = Sim::from_grid(track(), &[car(35.0,0.0,24.0), car(60.0,0.0,12.0), car(65.0,4.0,12.0)]);
    ...
    for tick in 0..600 {
        ...
        let inputs: Vec<_> = before.iter().map(|s| CarInput {
            throttle: ((12.0 - s.forward_speed) / 4.0).clamp(0.0,1.0), ..Default::default()
        }).collect();
    ...
    panic!("pass failed: {:?}, contacts={contacts}, severity={severity}", sim.pass_stats(0));
}
...
fn late_braking_in_a_narrow_corridor_follows_without_contact() {
    ...
    for _ in 0..300 {
        ...
        let snaps = sim.tick(&[CarInput::default(), CarInput { throttle: ((5.0 - speed)/4.0).clamp(0.0,1.0), ..Default::default() }]);
        min_gap = min_gap.min(snaps[1].pose.x - snaps[0].pose.x);
        max_contact = max_contact.max(snaps[0].car_contact_speed);
        assert!(!snaps[0].car_contact, "late approach contacted at gap={min_gap}, severity={max_contact}");
...
fn moving_rejoin_aborts_the_pass_without_contact() {
    let rejoin = GridCar { pose: Vec2::new(56.0,-12.0), heading: 0.65, velocity: Vec2::new(6.4,4.8) };
    let mut sim = Sim::from_grid(track(), &[car(35.0,0.0,20.0), car(60.0,0.0,10.0), rejoin]);
    sim.set_ai(0, Some(AiDriver::new(0)));
    let mut near = 0;
    let mut max_contact = 0.0_f32;
    for _ in 0..150 {
        ...
        let snaps = sim.tick(&[CarInput::default(),
            CarInput { throttle: ((10.0-before[1].forward_speed)/4.0).clamp(0.0,1.0), ..Default::default() },
            CarInput { throttle: ((8.0-before[2].forward_speed)/4.0).clamp(0.0,1.0), ..Default::default() }]);
        near += usize::from(snaps[0].pose.distance(snaps[1].pose) < 28.0);
        max_contact = max_contact.max(snaps[0].car_contact_speed);
        assert!(!snaps[0].car_contact, "rejoin contact severity={max_contact}");
    }
    let stats = sim.pass_stats(0).unwrap();
    eprintln!("moving rejoin: {stats:?}, player contacts=0, severity={max_contact}, near_ticks={near}");
